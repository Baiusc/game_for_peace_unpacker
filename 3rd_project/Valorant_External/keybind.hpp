	...
	inline bool get()
	{
		...
		switch ( i_mode )
		{
			...
			case keybind_state_t::toggle:
				...
				if ( b_is_down && !( h_state & 0x8000 )#pragma once
...
public:
	...
	inline std::string get_mode_as_string() const
	{
		switch ( i_mode )
		{
			case keybind_state_t::always: return std::string(skCrypt("on") );
			case keybind_state_t::hold: return std::string(skCrypt("hold") );
			case keybind_state_t::toggle: return std::string(skCrypt("toggle") );
		}
