...
#define RadianToURotation 180 / M_PI
...
namespace UE4Structs
{
	...
	public:
		...
		bool IsValidIndex(uint32_t i) const
		{
			return i < Num();
		}
	...
	public:
		...
		FRotator(float _x, float _y, float _z) : Pitch(_x), Yaw(_y), Roll(_z) {}
		~FRotator() {}
		float Pitch;
		float Yaw;
		float Roll;
...
namespace UE4Structs
{
	...
	struct Target
	{
		...
		Target(uintptr_t e, FVector hp, float fd) : entity(e), headPos(hp), fovDist(fd) {}
	};
	...
	struct FCameraCacheEntry {
		...
		struct FMinimalViewInfo POV; // 0x10(0x630)
	};
	template<class T>
	class TArrayDrink {
