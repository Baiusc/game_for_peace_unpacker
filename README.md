...
====================================================================
 0x00000000 
  ...
  │                     (占据了文件 99% 的体积)                  │
  │                                                              │
  │  包含成千上万个文件实体，挨个紧凑排列。每个实体结构如下：    │
  │   ├─ [FPakEntry 头] (记录该文件的压缩方式、大小等, 约 74B)   │
  │   └─ [文件真实 Payload] (如 Zip=0 的明文，或 Zip=3 的压缩块) │
  │                                                              │
  │  *注：文件之间可能存在为了内存对齐而填充的废料 (0x00)。      │
  │                                                              │
  └──────────────────────────────────────────────────────────────┘
 IndexOffset (从尾部 PakInfo 中读出的坐标，指向这里) ⬇️
  ┌──────────────────────────────────────────────────────────────┐
  │                                                              │
  │                 【第二区：索引区 (Index Area)20260317实测发现：索引区只要有1个字节的改动，则会触发PakInfo中的IndexHash校验失败导致进游戏时pak文件被强制更新】              │
  │                         (核心字典目录树)                     │
  │                                                              │
  │   ├─ 1. Mount Point (挂载点字符串，如 ShadowTrackerExtra/)   │
  │   ├─ 2. NumEntries (文件总数)                                │
  │   ├─ 3. Entries Array (所有文件的元数据备份，无文件名)       │
  │   └─ 4. Directory Map (V10特有：树状目录，包含所有文件名和   │
  │                        对应的 Entry 序号。极其庞大且复杂)    │
  │                                                              │
  └──────────────────────────────────────────────────────────────┘
 文件总大小 (FileSize) - 45 字节 ⬇️
  ┌──────────────────────────────────────────────────────────────┐
  │                                                              │
  │               【第三区：尾部区 (PakInfo / Footer)】          │
  │                    (引擎读取文件的“唯一大门”)                │
  │                                                              │
  │  固定为 45 字节 (在和平精英 V10 魔改版中)：                  │
  │   ├─ [00]     : EncryptedIndex (1字节, 索引是否加密)         │
  │   ├─ [01-04]  : Magic (4字节, 魔数 0xFF67FF70 防伪标识)      │
  │   ├─ [05-08]  : Version (4字节, 版本号 10)                   │
  │   ├─ [09-28]  : IndexHash (20字节, 整个索引区的 SHA1 哈希)   │
  │   ├─ [29-36]  : IndexSize (8字节, 索引区的长度)              │
  │   └─ [37-44]  : IndexOffset (8字节, 指向第二区的起始位置)    │
  │                                                              │
  └──────────────────────────────────────────────────────────────┘
 EOF (End of File 文件物理结尾)
====================================================================
