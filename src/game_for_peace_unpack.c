...
int main(int argc, const char *argv[]) {
    ...
    for (int files = 0; files < DIR_COUNT; files++) {
        ...
        for (int x = 0; x < DIR_FILES; x++) {
            ...
            // 若目标特征符合，则保存目标文件相关的信息以及ENTRY为本地txt文件
            if (strcmp(Filename, "BP_PlayerRifleBullet.uasset") == 0)
            {
                FILE *logFile = fopen("target_file_info.txt", "w");
                if (logFile == NULL) {
                    fprintf(stderr, "Error opening log file: target_file_info.txt\n");
                } else {
                    fprintf(logFile, "Found target file: %s\n", path);
