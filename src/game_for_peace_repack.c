...
int SerializeDirMap(DirMapEntry *dir_map)
{
    ...
    if (!dir_map->dir_bin || !dir_map->dir_bin_head || !dir_map->dir_bin_body)
    {
        ...
        if (dir_map->dir_bin_head)
            free(dir_map->dir_bin_head);
        if (dir_map->dir_bin_body)
            free(dir_map->dir_bin_body);
...
void SerializeHeadData(Entry *entry, uint8_t **out_head, size_t *out_size)
{
    ...
    *out_head = (uint8_t *)malloc(94);
    ...
    // 1. 写入 FileHash (20B)
    memcpy(ptr, entry->FileHash, 20);
    ptr += 20;

    // 2. 写入 FileOffset (8B)
    memcpy(ptr, &entry->FileOffset, 8);
    ptr += 8;

    // 3. 写入 FileSize (8B)
    memcpy(ptr, &entry->FileSize, 8);
    ptr += 8;

    // 4. 写入 CompressionMethod (4B)
    memcpy(ptr, &entry->CompressionMethod, 4);
    ptr += 4;

    // 5. 写入 CompressedLength (8B)
    memcpy(ptr, &entry->CompressedLength, 8);
    ptr += 8;

    // 6. 写入 Dummy (21B)
    memcpy(ptr, entry->Dummy, 21);
    ptr += 21;

    // 7. 写入 NumOfBlocks (4B)
    memcpy(ptr, &entry->NumOfBlocks, 4);
    ptr += 4;

    // 8. 写入 第一个 CompressionStart 和 End (8+8=16B)
    for (uint32_t i = 0; i < entry->NumOfBlocks; i++)
    {
        memcpy(ptr, &entry->blocks[i].start, 8);
        ptr += 8;
        memcpy(ptr, &entry->blocks[i].end, 8);
        ptr += 8;
    }

    // 9. 写入 CompressedBlockSize (4B)
    memcpy(ptr, &entry->CompressedBlockSize, 4);
    ptr += 4;

    // 10. 写入 Encrypted (1B)
    memcpy(ptr, &entry->Encrypted, 1);
    ptr += 1;
...
int ParsePakIndex(int PakFileDescriptor, PakIndexData *Result)
{
    ...
    for (uint64_t dir_idx = 0; dir_idx < DIR_COUNT; dir_idx++)
    {
        ...
        read_data(DIR_NAME, Result->OriginalIndexData, DIR_LEN);
        DIR_NAME[DIR_LEN] = '\0';
...
{
    if (data->OriginalIndexData)
        free(data->OriginalIndexData);
    if (data->FileEntries)
    {
        for (uint32_t i = 0; i < data->NumOfEntry; i++)
        {
            if (data->FileEntries[i].blocks)
                free(data->FileEntries[i].blocks);
        }
    ...
    if (data->AllFileInstances)
    {
        ...
        {
            if (data->AllFileInstances[i].DirPath)
                free(data->AllFileInstances[i].DirPath);
        }
...
void VerifyNewIndexData(const uint8_t *NewIndexData, uint64_t NewIndexDataSize,
                        uint32_t NumOfEntry, const uint32_t old_entry_indices[2])
{
    printf("\n\n--- 🌟 调试验证：解析内存中的新索引数据 🌟 ---\n");
    ...
    for (uint32_t i = 0; i < read_NumOfEntry; i++)
    {
        ...
        // 验证 FileHash (20 字节)
        if (verify_offset + 20 > NewIndexDataSize)
            break;
        ...
        if (verify_offset + 8 + 21 > NewIndexDataSize)
            break;
        verify_offset += 8 + 21;
        ...
        if (CompressionMethod != 0)
        {
            ...
            if (verify_offset + blocks_size > NewIndexDataSize)
                break;
            verify_offset += blocks_size;
        }

        // 跳过 CompressedBlockSize (4B) 和 Encrypted (1B)
        if (verify_offset + 4 + 1 > NewIndexDataSize)
            break;
        verify_offset += 4 + 1;
    }
    ...
    for (uint64_t d = 0; d < DIR_COUNT; d++)
    {
        ...
        uint64_t temp_offset = verify_offset; // 临时偏移，用于预扫描文件条目

        for (uint64_t f = 0; f < DIR_FILES; f++)
        {
            if (temp_offset + 4 > NewIndexDataSize)
                break;
            uint32_t FilenameSize;
            memcpy(&FilenameSize, NewIndexData + temp_offset, 4);
            temp_offset += 4;

            int raw_name_len_int = (int32_t)FilenameSize;
            int raw_name_len = (raw_name_len_int > 0) ? raw_name_len_int : -raw_name_len_int * 2;
            if (raw_name_len < 0 || temp_offset + raw_name_len > NewIndexDataSize)
                break;
            temp_offset += raw_name_len;

            if (temp_offset + 4 > NewIndexDataSize)
                break;
            uint32_t ENTRY_Index;
            memcpy(&ENTRY_Index, NewIndexData + temp_offset, 4);
            temp_offset += 4;

            // 检查是否接近 old_entry_indices[0] 或 [1]
            if ((ENTRY_Index >= old_entry_indices[0] && ENTRY_Index <= old_entry_indices[0] + 10) ||
                (ENTRY_Index >= old_entry_indices[0] - 10 && ENTRY_Index <= old_entry_indices[0]) ||
                (ENTRY_Index >= old_entry_indices[1] && ENTRY_Index <= old_entry_indices[1] + 10) ||
                (ENTRY_Index >= old_entry_indices[1] - 10 && ENTRY_Index <= old_entry_indices[1]))
            {
                is_need_print_dir = 1;
                break; // 找到一个即可
            }
            // 检查是否接近 old_entry_indices[0] 或 [1]，或处于全局首尾10个条目
            if (ENTRY_Index <= 10 ||
                ENTRY_Index >= NumOfEntry - 10 ||
                (ENTRY_Index >= old_entry_indices[0] - 10 && ENTRY_Index <= old_entry_indices[0] + 10) ||
                (ENTRY_Index >= old_entry_indices[1] - 10 && ENTRY_Index <= old_entry_indices[1] + 10))
            {
                is_need_print_dir = 1;
                break; // 找到一个即可
            }
        ...
        for (uint64_t f = 0; f < DIR_FILES; f++)
        {
            ...
            int raw_name_len_int = (int32_t)FilenameSize;
            int raw_name_len = (raw_name_len_int > 0) ? raw_name_len_int : -raw_name_len_int * 2;
            if (raw_name_len < 0 || verify_offset + raw_name_len > NewIndexDataSize)
            {
                goto dir_map_error;
            }
            ...
            if (FilenameSize > 0)
            {
                size_t copy_len = (raw_name_len < (int)sizeof(filename_utf8) - 1) ? raw_name_len : sizeof(filename_utf8) - 1;
...
int main()
{
    ...
    const char *NEW_PAK_PATH = "../paks/map_weapon_1.34.12.14500原厂.pak"; // 🌟 新增：生成的新文件路径
    ...
    if (SrcPakFile == -1)
    {
        fprintf(stderr, "Failed to open src.pak\n");
        return 1;
    }
    ...
    if (MyPakFile == -1)
    {
        fprintf(stderr, "Failed to open my.pak\n");
        result = 1;
        goto cleanup;
    }
    ...
    if (NewPakFile == -1)
    {
        fprintf(stderr, "Failed to create new.pak\n");
        result = 1;
        goto cleanup;
    }
    ...
    VerifyNewIndexData(NewIndexData, NewIndexDataSize, src_data.NumOfEntry, old_entry_indices);
...
cleanup:
    ...
    for (int i = 0; i < 2; i++)
    {
        if (my_instances[i].Data)
            free(my_instances[i].Data);
