...
${this.isEnum ? `enum` : this.isStruct ? `struct` : this.isInterface ? `interface` : `class`} ${this.type.name}${inherited ? ` : ${inherited.map((_) => _?.type.name).join(`, `)}` : ``}
{
  ...
  var ALLOW_IS_DEAD = CFG.allowIsDead === true;
  var MAIN_THREAD = CFG.mainThread !== false;
  var ASM = "Assembly-CSharp";
...
[discover] ${label} (${klass.name})`);
  ...
  console.log(`[*] frida agent loaded, waiting for IL2CPP module ... (level=${LEVEL}: ${LEVEL_DESC[LEVEL] || "?"}, interval=${INTERVAL_MS}ms)`);
  if (LEVEL <= 0) {
    let alive = 0;
    setInterval(() => {
      alive++;
...
`;
      ...
      const gmFromHeapScan = () => {
        const now = Date.now();
        if (alive(gmFromHeap) && now - gmHeapTs < 5e3) return gmFromHeap;
        gmHeapTs = now;
        try {
      ...
      const getGameManager = () => gmFromStaticField() || gmFromHeapScan() || (ALLOW_GET_INSTANCE ? gmFromGetInstance() : null);
      const getMyPlayer = () => {
        try {
          const v = GameManager.field("myPlayer").value;
          return alive(v) ? v : null;
        } catch (e) {
          return null;
        }
      ...
      const getAllPlayers = (gm) => {
        try {
          const a = gm.field("allPlayers").value;
      ...
      const readObscuredInt = (owner, fieldName) => {
        try {
          ...
          const key = base.readS32();
          const hidden = base.add(4).readS32();
          const inited = base.add(8).readU8();
          const fake = base.add(12).readS32();
          const fakeActive = base.add(16).readU8();
      ...
      const TEAM_FIELD = "<team>k__BackingField";
      const TEAM_OFFSET_FALLBACK = 28;
      const readTeamOf = (p) => {
        try {
      ...
      const readPlayer = (p, withHp) => {
        ...
        try {
          out.isMyPlayer = callMethod(p, "get_isMyPlayer", 0);
        } catch (e) {
          out.isMyPlayer = null;
        }
      ...
      const frameTick = () => {
        ...
        if (allPlayers) {
          ...
          for (let i = 0; i < n; i++) {
            let p = null;
            try {
              p = useGet ? allPlayers.get(i) : allPlayers[i];
            } catch (e) {
              p = null;
            }
