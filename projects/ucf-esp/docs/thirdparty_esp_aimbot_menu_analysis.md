...
> 上的合法实现（只算角度、不写游戏内存/不 Hook、不碰第三方进程）。第三方项目印证了该算法范式的通用性。
> 本地鼠标模拟（SendInput/mouse_event，含 triggerbot 式自动左键）按根 AGENTS.md §0.1 授权，为独立 `input_sim/` 模块、默认 OFF。
...
| **对 ucf-esp 的启示** | 不采用「写游戏内存/Hook/注入到游戏进程」；本地鼠标模拟（§0.1 授权、独立模块、默认 OFF）可直接对标 | **我们正是外部叠加式**，范式一致，可直接对标 |
...
注入/Hook 游戏、DMA/驱动读内存、特征码/偏移、反作弊规避、autowall、
**直接写游戏视角内存**的「路径 B」瞄准。这些与 ucf-esp「单机调试、不写游戏内存、不碰第三方进程」的定位冲突。
（注：triggerbot 式自动左键属本地鼠标模拟，按根 AGENTS.md §0.1 授权、独立 `input_sim/` 模块、默认 OFF 时允许。）
