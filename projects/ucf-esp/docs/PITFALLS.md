...
### 2026-09-18：aimbot target=-1 的两个运行时原因

- **现象**：菜单显示 `AIM:ON target:-1`，侧键和目标射线均无反应，真实帧中的 `bones[19]` 可能全部 `valid=false`。
- **根因**：取点默认是头部，旧逻辑把缺失骨骼直接视为候选无效；同时旧输入状态机只读取瞄准侧键，配置中的触发侧键没有参与边沿检测。
